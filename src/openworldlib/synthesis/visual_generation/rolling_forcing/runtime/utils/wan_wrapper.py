...
DEFAULT_WAN_MODEL_PATH = Path("checkpoints/Wan2.1-T2V-1.3B")


class WanTextEncoder(torch.nn.Module):
    def __init__(self, model_path: str | Path | None = None, device: torch.device | str | None = None) -> None:
        super().__init__()
        self.model_path = Path(model_path) if model_path is not None else DEFAULT_WAN_MODEL_PATH
        self.runtime_device = torch.device(device) if device is not None else (
        ...
        self.text_encoder.load_state_dict(
            torch.load(self.model_path / "models_t5_umt5-xxl-enc-bf16.pth",
                       map_location='cpu', weights_only=False)
        )
        self.text_encoder = self.text_encoder.to(self.runtime_device)

        self.tokenizer = HuggingfaceTokenizer(
            name=str(self.model_path / "google/umt5-xxl"), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, model_path: str | Path | None = None):
        ...
        self.model = _video_vae(
            pretrained_path=str(self.model_path / "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        model_root = Path(model_path) if model_path is not None else Path("checkpoints") / model_name

        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                str(model_root), local_attn_size=local_attn_size, sink_size=sink_size)
        else:
            self.model = WanModel.from_pretrained(str(model_root))
        self.model.eval()
