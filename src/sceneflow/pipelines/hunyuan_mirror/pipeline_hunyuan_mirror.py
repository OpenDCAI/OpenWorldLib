...
from ...representations.hunyuan_mirror.mirror_src.models.models.worldmirror import WorldMirror
from ...representations.hunyuan_mirror.mirror_src.utils.inference_utils import prepare_images_to_tensor
from ...representations.hunyuan_mirror.mirror_src.utils.video_utils import video_to_image_frames
from ...representations.hunyuan_mirror.mirror_src.utils.save_utils import save_depth_png, save_depth_npy, save_normal_png
from ...representations.hunyuan_mirror.mirror_src.utils.save_utils import save_scene_ply, save_gs_ply, save_points_ply
from ...representations.hunyuan_mirror.mirror_src.utils.render_utils import render_interpolated_video
from ...representations.hunyuan_mirror.mirror_src.utils.build_pycolmap_recon import build_pycolmap_reconstruction
from ...representations.hunyuan_mirror.mirror_src.models.utils.camera_utils import vector_to_camera_matrices
from ...representations.hunyuan_mirror.mirror_src.utils.geometry import depth_edge, normals_edge
from ...representations.hunyuan_mirror.mirror_src.utils.visual_util import segment_sky, download_file_from_url
from ...representations.hunyuan_mirror.mirror_src.models.utils.geometry import depth_to_world_coords_points
from ...representations.hunyuan_mirror.mirror_src.models.utils.geometry import create_pixel_coordinate_grid
...
class HunyuanMirrorPipeline:
    ...
    def process_images(self, 
        ...
        if "pts3d" in predictions and save_pointmap:
            ...
            save_scene_ply(ply_path, filtered_pts, filtered_colors)
            print(f"  - Saved {len(filtered_pts)} filtered points to {ply_path}")
            results['pointmap_path'] = str(ply_path)
        ...
        if "depth" in predictions and save_depth:
            ...
            results['depth_dir'] = str(depth_dir)
        ...
        if "normals" in predictions and save_normal:
            ...
            results['normal_dir'] = str(normal_dir)
        ...
        if "splats" in predictions and save_gs:
            ...
            results['gaussians_path'] = str(ply_path)
            results['rendered_video_path'] = str(self.output_path / "rendered") if save_rendered else None
    ...
    def __call__(self,
        ...
        Args:
            ...
            save_pointmap: 是否保存点云
            save_depth: 是否保存深度图
            save_normal: 是否保存法线图
            save_gs: 是否保存高斯分布
            save_rendered: 是否保存渲染视频
            save_colmap: 是否保存COLMAP重建
