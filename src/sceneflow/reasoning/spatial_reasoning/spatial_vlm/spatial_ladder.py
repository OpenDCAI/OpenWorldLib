...
class SpatialLadder(BaseReasoning):
    ...
    def inference(
        ...
        """
        if messages is None:
            batched_messages = [self._build_messages(image_paths=image_paths, instruction=instruction)]
        else:
            if not messages:
        ...
        image_inputs = [vi[0] for vi in vision_info]
        video_inputs = [vi[1] for vi in vision_info]
