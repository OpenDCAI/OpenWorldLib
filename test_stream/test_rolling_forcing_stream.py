...
output_path = os.environ.get("ROLLING_FORCING_STREAM_OUTPUT_PATH", "rolling_forcing_stream_demo.mp4")
num_frames = int(os.environ.get("ROLLING_FORCING_STREAM_NUM_FRAMES", "126"))
...
first = pipeline.stream(
    prompt="A cinematic shot of a silver train crossing a desert at sunrise.",
    num_frames=num_frames,
    seed=0,
    reset=True,
)
second = pipeline.stream(
    prompt="The same train continues into a glowing futuristic city at dusk.",
    num_frames=num_frames,
    seed=1,
)

output_video = torch.cat([first, second], dim=1)
save_uint8_video(output_video, output_path, fps=fps)
print(f"Done! Stream video saved to {output_path}")
